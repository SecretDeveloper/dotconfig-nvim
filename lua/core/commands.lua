...
user_cmd("CopyRange", function(opts)
  local mark_a = tostring(opts.fargs[1]) or ""
  local mark_b = tostring(opts.fargs[2]) or ""
  require("core.utils").copy_range(mark_a, mark_b)
end, {
  desc = "Copy from mark a to mark b",
  nargs = "?",
  bang = true,
})
